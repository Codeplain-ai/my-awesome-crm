...
def test_get_contacts_empty(client: TestClient):
...
def test_get_contacts_search_and_pagination(client: TestClient, session: Session):
    # Seed data
    ...
    # Test search 'Smith'
    response = client.get("/contacts?q=Smith")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 2
...
def test_get_contacts_limit_validation(client: TestClient):
    # Limit 201 should trigger FastAPI validation error (422)
    response = client.get("/contacts?limit=201")
    assert response.status_code == 422

def test_get_contact_by_id(client: TestClient, session: Session):
    # Seed contact
